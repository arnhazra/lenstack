...
export class OrganizationRepository {
  constructor(
    @InjectModel(Organization.name, DbConnectionMap.Core)
  ...
  async findOne<K extends keyof Organization>(
    filter: Pick<Organization, K>
  ): Promise<Organization | null> {
    return await this.model.findOne(filter as FilterQuery<Organization>)
  }

  async findAll<K extends keyof Organization>(
    filter: Pick<Organization, K>
  ): Promise<Organization[] | null> {
    return await this.model.find(filter as FilterQuery<Organization>)
  }

  async deleteById(orgId: Types.ObjectId): Promise<Organization | null> {
